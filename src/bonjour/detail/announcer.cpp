...
namespace bonjour {
    namespace detail {
        ...
        {
            static void DNSSD_API on_registered(::DNSServiceRef, ::DNSServiceFlags flags, ::DNSServiceErrorType error,
                ...
                try {
                    if(error == kDNSServiceErr_NoError) {
                        if(flags & kDNSServiceFlagsAdd) {
                            ...
                            self->current->contact = aware::contact(aware::detail::type_decode(regtype)).name(name);
        ...
        announcer::current_type::current_type(aware::contact& contact, async_announce_handler handler)
            : contact(contact), handler(handler) {}

        announcer::current_type::current_type(const current_type& other)
            : contact(other.contact), properties(other.properties), handler(other.handler) {}
