    ...
    namespace detail {
        ...
        void monitor::on_browser_appear(const aware::contact& contact, bool commit) {
            ...
            if(commit) {
                ...
                for(name_container::iterator it = uncommitted_appear.begin(); it != uncommitted_appear.end(); ++it) {
                    if(*it == entry->first)
                        continue;
        ...
        void monitor::on_browser_disappear(const aware::contact& contact, bool commit) {
            ...
            if(commit) {
                ...
                for(name_container::iterator it = uncommitted_disappear.begin(); it != uncommitted_disappear.end();
                    ++it) {
                    if(*it == entry->first)
                        continue;
        ...
        monitor::scope::scope(boost::asio::io_service& io, monitor& self) : self(self), timer(io), wildcard_count(0) {}
        ...
        void monitor::scope::commit_appear() {
            ...
            std::set_intersection(additions.begin(), additions.end(), removals.begin(), removals.end(),
        ...
        void monitor::scope::commit_disappear() {
            ...
            timer.async_wait(boost::bind(&monitor::scope::process_disappear, this, boost::asio::placeholders::error));
        }
        ...
        void monitor::scope::execute_appear() {
            if(wildcard_count > 0) {
                ...
                std::set_difference(active.begin(), active.end(), additions.begin(), additions.end(),
