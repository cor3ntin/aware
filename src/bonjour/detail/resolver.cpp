    ...
    namespace detail {
        ...
        boost::asio::ip::address to_address(const sockaddr& addr) {
            ...
            switch(addr.sa_family) {
                case AF_INET:
                    return ip::address_v4(reinterpret_cast<const ip::address_v4::bytes_type&>(
                        reinterpret_cast<const sockaddr_in&>(addr).sin_addr));

                case AF_INET6:
                    return ip::address_v6(reinterpret_cast<const ip::address_v6::bytes_type&>(
                ...
                try {
                    if(error == kDNSServiceErr_NoError) {
                        ...
                        for(uint16_t current = 0; current < count; ++current) {
                            if(::TXTRecordGetItemAtIndex(txt_length, txt_record, current, keyLength, key, &valueLength,
                                                         &value) == kDNSServiceErr_NoError) {
                                properties[key] = std::string(static_cast<const char*>(value), valueLength);
                            }
                        }
            ...
            static void on_addrinfo(::DNSServiceRef, ::DNSServiceFlags flags, uint32_t /* interface_index */,
                ...
                try {
                    if(error == kDNSServiceErr_NoError) {
                        ...
                        boost::asio::ip::tcp::endpoint endpoint(bonjour::detail::to_address(*address), self->port);
        ...
        resolver::resolver(detail::handle& connection, const aware::contact& contact, resolver_listener& listener)
            ...
            ::DNSServiceErrorType error =
                ::DNSServiceResolve(&ref, flags, from_index(contact.index()), contact.name().c_str(), regtype.c_str(),
                                    contact.domain().c_str(), &resolver::callback::on_resolved, this);
        ...
        void resolver::on_resolved(const char* host) {
            const ::DNSServiceFlags flags = kDNSServiceFlagsShareConnection;
            const ::DNSServiceProtocol protocol = (contact.endpoint().protocol() == boost::asio::ip::tcp::v6()) ?
                kDNSServiceProtocol_IPv6 :
                kDNSServiceProtocol_IPv4;

            ::DNSServiceRef ref = connection.get<DNSServiceRef>();
            ::DNSServiceErrorType error = ::DNSServiceGetAddrInfo(&ref, flags, from_index(contact.index()), protocol,
                                                                  host, &resolver::callback::on_addrinfo, this);
            throw_on_error(error);
